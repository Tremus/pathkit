...
#include "pathkit/private/SkTemplates.h"
...
public:
     ...
     */
    T& operator[](int i) { return fItemArray[i]; }

    const T& operator[](int i) const { return fItemArray[i]; }
     ...
     */
    T& fromBack(int i) { return fItemArray[fCount - i - 1]; }

    const T& fromBack(int i) const { return fItemArray[fCount - i - 1]; }
