...
SkPath::SkPath(sk_sp<SkPathRef> pr,
               SkPathFillType ft,
               SkPathConvexity ct,
    ...
    if (matrix.hasPerspective()) {
        ...
        while ((verb = iter.next(pts)) != kDone_Verb) {
            switch (verb) {
                ...
                case kConic_Verb:
                    tmp.conicTo(
                            pts[1], pts[2], SkConic::TransformW(pts, iter.conicWeight(), matrix));
                    break;
                case kCubic_Verb:
...
bool SkPathPriv::IsRectContour(const SkPath& path,
    ...
    while (*currVerb < verbCnt && (!allowPartial || !autoClose)) {
        uint8_t verb =
                insertClose ? (uint8_t)SkPath::kClose_Verb : path.fPathRef->atVerb(*currVerb);
        switch (verb) {
            case SkPath::kClose_Verb:
                savePts = pts;
                autoClose = true;
    ...
    if (direction) {
        *direction = directions[0] == ((directions[1] + 1) & 3) ? SkPathDirection::kCW
