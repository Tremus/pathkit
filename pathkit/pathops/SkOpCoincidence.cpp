...
#include "pathkit/pathops/SkPathOpsTSect.h"
...
bool SkCoincidentSpans::ordered(bool* result) const {
    ...
    do {
        ...
        if (!opp) {
            //            PkOPOBJASSERT(start, 0);  // may assert if coincident span isn't fully
            //            processed
            return false;
        }
...
*/
double SkOpCoincidence::TRange(const SkOpPtT* overS,
                               double t,
    ...
    if (!Ordered(coinSeg, oppSeg)) {
        if (oppTs < oppTe) {
            return this->checkOverlap(
                    check, oppSeg, coinSeg, oppTs, oppTe, coinTs, coinTe, overlaps);
        }
...
bool SkOpCoincidence::addIfMissing(const SkOpPtT* over1s,
...
bool SkOpCoincidence::addOrOverlap(SkOpSegment* coinSeg,
                                   SkOpSegment* oppSeg,
                                   double coinTs,
                                   double coinTe,
                                   double oppTs,
        ...
        while ((inner = inner->next())) {
            ...
            if (outerCoin == innerCoin) {
                ...
                if (outerOpp != innerOpp && this->overlap(ocs, oce, ics, ice, &overS, &overE)) {
                    FAIL_IF(!this->addIfMissing(ocs->starter(oce),
                                                ics->starter(ice),
                                                overS,
                                                overE,
                                                outerOppWritable,
                                                innerOppWritable,
                                                added PkDEBUGPARAMS(ocs->debugEnder(oce))
                ...
                if (this->overlap(oos, ooe, ics, ice, &overS, &overE)) {
                    FAIL_IF(!this->addIfMissing(oos->starter(ooe),
                                                ics->starter(ice),
                                                overS,
                                                overE,
                                                outerCoinWritable,
                                                innerOppWritable,
                                                added PkDEBUGPARAMS(oos->debugEnder(ooe))
                                                        PkDEBUGPARAMS(ics->debugEnder(ice))));
                }
...
bool SkOpCoincidence::addOverlap(const SkOpSegment* seg1,
...
bool SkOpCoincidence::contains(const SkCoincidentSpans* coin,
        ...
        while ((inner = inner->next())) {
            ...
            if ((outerOpp == innerCoin && SkOpPtT::Overlaps(outer->oppPtTStart(),
                ...
                (outerOpp == innerOpp && SkOpPtT::Overlaps(outer->oppPtTStart(),
                                                           ...
                                                           &overlapS,
                                                           &overlapE))) {
                if (!overlaps->addOverlap(
                            outerCoin, outerOpp, innerCoin, innerOpp, overlapS, overlapE)) {
                    return false;
                }
            }
        }
...
bool SkOpCoincidence::overlap(const SkOpPtT* coin1s,
                              const SkOpPtT* coin1e,
                              const SkOpPtT* coin2s,
