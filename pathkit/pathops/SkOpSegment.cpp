...
#include "pathkit/pathops/SkPathWriter.h"
...
bool SkOpSegment::addCurveTo(const SkOpSpanBase* start,
    ...
    switch (verb) {
        ...
        case SkPath::kConic_Verb:
            path->conicTo(curvePart.fCurve.fConic[1].asSkPoint(),
                          end->ptT(),
                          curvePart.fCurve.fConic.fWeight);
            break;
        case SkPath::kCubic_Verb:
            path->cubicTo(curvePart.fCurve.fCubic[1].asSkPoint(),
                          curvePart.fCurve.fCubic[2].asSkPoint(),
                          end->ptT());
            break;
        default:
            PkASSERT(0);
...
bool SkOpSegment::ComputeOneSum(const SkOpAngle* baseAngle,
    ...
    if (binary) {
        ...
        nextSegment->setUpWindings(nextAngle->start(),
                                   nextAngle->end(),
                                   &sumMiWinding,
                                   &sumSuWinding,
                                   &maxWinding,
...
bool SkOpSegment::ComputeOneSumReverse(SkOpAngle* baseAngle,
    ...
    if (binary) {
        int oppMaxWinding, oppSumWinding;
        nextSegment->setUpWindings(nextAngle->end(),
                                   nextAngle->start(),
                                   &sumMiWinding,
                                   &sumSuWinding,
                                   &maxWinding,
...
SkOpSegment* SkOpSegment::findNextOp(SkTDArray<SkOpSpanBase*>* chase,
    ...
    do {
        ...
        bool activeAngle = nextSegment->activeOp(xorMiMask,
                                                 xorSuMask,
                                                 nextAngle->start(),
                                                 nextAngle->end(),
                                                 op,
        ...
        if (last) {
            ...
            *chase->append() = last;
#if DEBUG_WINDING
            SkDebugf("%s chase.append segment=%d span=%d",
                     __FUNCTION__,
                     last->segment()->debugID(),
                     last->debugID());
            if (!last->final()) {
...
SkOpSegment* SkOpSegment::findNextXor(SkOpSpanBase** nextStart,
    ...
    PkDEBUGCODE(SkOpSpanBase* endNear =
...
bool SkOpSegment::markAndChaseWinding(SkOpSpanBase* start,
    ...
    while ((other = other->nextChase(&start, &step, &spanStart, &last))) {
        ...
        if (spanStart->windSum() != PK_MinS32) {
            //            PkASSERT(spanStart->windSum() == winding);   // FIXME: is this assert too
            //            aggressive?
            PkASSERT(!last);
            break;
        }
...
bool SkOpSegment::markAngle(int maxWinding,
    ...
    if (result) {
        SkOpSpanBase* last = *result;
        if (last) {
            SkDebugf("%s last segment=%d span=%d",
                     __FUNCTION__,
                     last->segment()->debugID(),
                     last->debugID());
            if (!last->final()) {
...
bool SkOpSegment::match(const SkOpPtT* base,
                        const SkOpSegment* testParent,
                        double testT,
        ...
        while ((ptT = ptT->next()) != spanStopPtT) {
            ...
            if (this->testForCoincidence(rootPriorPtT, rootPtT, prior, spanBase, opp)) {
                ...
                SkDebugf("%s coinSpan=%d endSpan=%d oppSpan=%d oppEndSpan=%d\n",
                         __FUNCTION__,
                         rootPriorPtT->debugID(),
                         rootPtT->debugID(),
                         rootOppStart->debugID(),
                         rootOppEnd->debugID());
#endif
...
bool SkOpSegment::spansNearby(const SkOpSpanBase* refSpan,
                              const SkOpSpanBase* checkSpan,
                              bool* found) const {
...
doneCheckingDistance:
    *found = checkBest && refBest->segment()->match(
                                  refBest, checkBest->segment(), checkBest->fT, checkBest->fPt);
    return true;
}
...
int SkOpSegment::updateWinding(SkOpSpanBase* start, SkOpSpanBase* end) {
    ...
    if (winding && UseInnerWinding(winding - spanWinding, winding) && winding != PK_MaxS32) {
        winding -= spanWinding;
    }
    return winding;
}
