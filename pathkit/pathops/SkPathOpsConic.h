...
struct SkDConic {
    ...
    SkDConic flip() const {
        SkDConic result = {{{fPts[2], fPts[1], fPts[0]} PkDEBUGPARAMS(fPts.fDebugGlobalState)},
                           fWeight};
        return result;
    }
    ...
    const SkDConic& set(const SkPoint pts[kPointCount],
                        SkScalar weight PkDEBUGPARAMS(SkOpGlobalState* state = nullptr)) {
    ...
    static SkDPoint SubDivide(const SkPoint pts[kPointCount],
                              SkScalar weight,
                              const SkDPoint& a,
                              const SkDPoint& c,
                              double t1,
