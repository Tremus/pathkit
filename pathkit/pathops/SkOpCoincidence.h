...
#include "pathkit/core/SkArenaAlloc.h"
#include "pathkit/pathops/SkOpSpan.h"
#include "pathkit/pathops/SkPathOpsTypes.h"
#include "pathkit/private/SkTDArray.h"
...
public:
...
#ifdef PK_DEBUG
    void debugStartCheck(const SkOpSpanBase* outer,
                         const SkOpSpanBase* over,
                         const SkOpGlobalState* debugState) const;
#endif
    void dump() const;
    bool expand();
    bool extend(const SkOpPtT* coinPtTStart,
    ...
    void set(SkCoincidentSpans* next,
...
public:
    ...
    bool contains(const SkOpPtT* coinPtTStart,
                  const SkOpPtT* coinPtTEnd,
                  const SkOpPtT* oppPtTStart,
                  const SkOpPtT* oppPtTEnd) const;
...
#if DEBUG_COIN
    void debugRelease(SkPathOpsDebug::GlitchLog*,
                      const SkCoincidentSpans*,
...
private:
    void add(const SkOpPtT* coinPtTStart,
             const SkOpPtT* coinPtTEnd,
             const SkOpPtT* oppPtTStart,
             const SkOpPtT* oppPtTEnd) {
        this->add(const_cast<SkOpPtT*>(coinPtTStart),
                  const_cast<SkOpPtT*>(coinPtTEnd),
                  const_cast<SkOpPtT*>(oppPtTStart),
                  const_cast<SkOpPtT*>(oppPtTEnd));
    }

    bool addEndMovedSpans(const SkOpSpan* base, const SkOpSpanBase* testSpan);
    bool addEndMovedSpans(const SkOpPtT* ptT);

    bool addIfMissing(const SkOpPtT* over1s,
                      const SkOpPtT* over2s,
                      double tStart,
                      double tEnd,
                      SkOpSegment* coinSeg,
                      SkOpSegment* oppSeg,
                      bool* added PkDEBUGPARAMS(const SkOpPtT* over1e)
                              PkDEBUGPARAMS(const SkOpPtT* over2e));
    bool addOrOverlap(SkOpSegment* coinSeg,
                      SkOpSegment* oppSeg,
                      double coinTs,
                      double coinTe,
                      double oppTs,
                      double oppTe,
                      bool* added);
    bool addOverlap(const SkOpSegment* seg1,
                    const SkOpSegment* seg1o,
                    const SkOpSegment* seg2,
                    const SkOpSegment* seg2o,
                    const SkOpPtT* overS,
                    const SkOpPtT* overE);
    ...
    bool overlap(const SkOpPtT* coinStart1,
                 const SkOpPtT* coinEnd1,
                 const SkOpPtT* coinStart2,
                 const SkOpPtT* coinEnd2,
                 double* overS,
                 double* overE) const;
    ...
    static double TRange(const SkOpPtT* overS,
                         double t,
                         const SkOpSegment* coinPtT PkDEBUGPARAMS(const SkOpPtT* overE));
