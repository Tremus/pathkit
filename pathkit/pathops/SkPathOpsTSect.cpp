...
#include "pathkit/core/SkTSort.h"
...
void SkTCoincident::setPerp(const SkTCurve& c1, double t, const SkDPoint& cPt, const SkTCurve& c2) {
    ...
    SkDebugf("setPerp t=%1.9g cPt=(%1.9g,%1.9g) %s oppT=%1.9g fPerpPt=(%1.9g,%1.9g)\n",
             ...
             cPt.fX,
             cPt.fY,
             cPt.approximatelyEqual(fPerpPt) ? "==" : "!=",
             fPerpT,
             fPerpPt.fX,
             fPerpPt.fY);
#endif
...
const SkTSect* SkTSpan::debugOpp() const { return PkDEBUGRELEASE(fDebugSect->debugOpp(), nullptr); }
...
SkTSect::SkTSect(const SkTCurve& c PkDEBUGPARAMS(SkOpGlobalState* debugGlobalState)
        ...
        , fDeleted(nullptr)
        , fActiveCount(0)
        , fHung(false) PkDEBUGPARAMS(fDebugGlobalState(debugGlobalState))
...
bool SkTSect::binarySearchCoin(SkTSect* sect2,
                               double tStart,
                               double tStep,
                               double* resultT,
                               double* oppT,
...
void SkTSect::coincidentForce(SkTSect* sect2, double start1s, double start1e) {
    ...
    double oppStartT =
            first->fCoinStart.perpT() == -1 ? 0 : std::max(0., first->fCoinStart.perpT());
...
struct SkClosestRecord {
    ...
    bool matesWith(const SkClosestRecord& mate PkDEBUGPARAMS(SkIntersections* i)) const {
        PkOPOBJASSERT(i,
                      fC1Span == mate.fC1Span || fC1Span->endT() <= mate.fC1Span->startT() ||
                              mate.fC1Span->endT() <= fC1Span->startT());
        PkOPOBJASSERT(i,
                      fC2Span == mate.fC2Span || fC2Span->endT() <= mate.fC2Span->startT() ||
...
struct SkClosestSect {
    ...
    bool find(const SkTSpan* span1, const SkTSpan* span2 PkDEBUGPARAMS(SkIntersections* i)) {
...
void SkTSect::BinarySearch(SkTSect* sect1, SkTSect* sect2, SkIntersections* intersections) {
    ...
    if (coincident) {
        ...
        do {
            ...
            int index = intersections->insertCoincident(
