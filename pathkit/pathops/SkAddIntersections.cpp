...
#include "pathkit/pathops/SkAddIntersections.h"
#include "pathkit/pathops/SkOpCoincidence.h"
#include "pathkit/pathops/SkPathOpsBounds.h"

namespace pk {
#if DEBUG_ADD_INTERSECTING_TS

static void debugShowLineIntersection(int pts,
    ...
    if (!pts) {
        SkDebugf("%s no intersect " LINE_DEBUG_STR " " LINE_DEBUG_STR "\n",
                 __FUNCTION__,
                 LINE_DEBUG_DATA(wt.pts()),
                 LINE_DEBUG_DATA(wn.pts()));
        return;
    }
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " LINE_DEBUG_STR " " PT_DEBUG_STR,
             __FUNCTION__,
             i[0][0],
             LINE_DEBUG_DATA(wt.pts()),
             PT_DEBUG_DATA(i, 0));
...
static void debugShowQuadLineIntersection(int pts,
                                          const SkIntersectionHelper& wt,
                                          const SkIntersectionHelper& wn,
                                          const SkIntersections& i) {
    PkASSERT(i.used() == pts);
    if (!pts) {
        SkDebugf("%s no intersect " QUAD_DEBUG_STR " " LINE_DEBUG_STR "\n",
                 __FUNCTION__,
                 QUAD_DEBUG_DATA(wt.pts()),
                 LINE_DEBUG_DATA(wn.pts()));
        return;
    }
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " QUAD_DEBUG_STR " " PT_DEBUG_STR,
             __FUNCTION__,
             i[0][0],
             QUAD_DEBUG_DATA(wt.pts()),
             PT_DEBUG_DATA(i, 0));
...
static void debugShowQuadIntersection(int pts,
                                      const SkIntersectionHelper& wt,
                                      const SkIntersectionHelper& wn,
                                      const SkIntersections& i) {
    PkASSERT(i.used() == pts);
...
static void debugShowConicLineIntersection(int pts,
                                           const SkIntersectionHelper& wt,
                                           const SkIntersectionHelper& wn,
                                           const SkIntersections& i) {
    PkASSERT(i.used() == pts);
    if (!pts) {
        SkDebugf("%s no intersect " CONIC_DEBUG_STR " " LINE_DEBUG_STR "\n",
                 __FUNCTION__,
                 CONIC_DEBUG_DATA(wt.pts(), wt.weight()),
                 LINE_DEBUG_DATA(wn.pts()));
        return;
    }
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " CONIC_DEBUG_STR " " PT_DEBUG_STR,
             __FUNCTION__,
             i[0][0],
             CONIC_DEBUG_DATA(wt.pts(), wt.weight()),
             PT_DEBUG_DATA(i, 0));
...
static void debugShowConicQuadIntersection(int pts,
                                           const SkIntersectionHelper& wt,
                                           const SkIntersectionHelper& wn,
                                           const SkIntersections& i) {
    PkASSERT(i.used() == pts);
...
static void debugShowConicIntersection(int pts,
                                       const SkIntersectionHelper& wt,
                                       const SkIntersectionHelper& wn,
                                       const SkIntersections& i) {
    PkASSERT(i.used() == pts);
...
static void debugShowCubicLineIntersection(int pts,
                                           const SkIntersectionHelper& wt,
                                           const SkIntersectionHelper& wn,
                                           const SkIntersections& i) {
    PkASSERT(i.used() == pts);
    ...
    SkDebugf("%s " T_DEBUG_STR(wtTs, 0) " " CUBIC_DEBUG_STR " " PT_DEBUG_STR,
             __FUNCTION__,
             i[0][0],
             CUBIC_DEBUG_DATA(wt.pts()),
             PT_DEBUG_DATA(i, 0));
...
static void debugShowCubicQuadIntersection(int pts,
                                           const SkIntersectionHelper& wt,
                                           const SkIntersectionHelper& wn,
                                           const SkIntersections& i) {
    PkASSERT(i.used() == pts);
...
static void debugShowCubicConicIntersection(int pts,
                                            const SkIntersectionHelper& wt,
                                            const SkIntersectionHelper& wn,
                                            const SkIntersections& i) {
    PkASSERT(i.used() == pts);
...
static void debugShowCubicIntersection(int pts,
                                       const SkIntersectionHelper& wt,
                                       const SkIntersectionHelper& wn,
                                       const SkIntersections& i) {
    PkASSERT(i.used() == pts);
...
static void debugShowConicIntersection(int,
                                       const SkIntersectionHelper&,
                                       const SkIntersectionHelper&,
                                       const SkIntersections&) {}
            ...
            switch (wt.segmentType()) {
                ...
                case SkIntersectionHelper::kVerticalLine_Segment:
                    ...
                    switch (wn.segmentType()) {
                        ...
                        case SkIntersectionHelper::kLine_Segment: {
                            pts = ts.lineVertical(
                                    wn.pts(), wt.top(), wt.bottom(), wt.x(), wt.yFlipped());
                            debugShowLineIntersection(pts, wn, wt, ts);
                            break;
                        }
                        case SkIntersectionHelper::kQuad_Segment: {
                ...
                case SkIntersectionHelper::kLine_Segment:
                    switch (wn.segmentType()) {
                        case SkIntersectionHelper::kHorizontalLine_Segment:
                            pts = ts.lineHorizontal(
                                    wt.pts(), wn.left(), wn.right(), wn.y(), wn.xFlipped());
                            debugShowLineIntersection(pts, wt, wn, ts);
                            break;
                        case SkIntersectionHelper::kVerticalLine_Segment:
                            pts = ts.lineVertical(
                                    wt.pts(), wn.top(), wn.bottom(), wn.x(), wn.yFlipped());
                            debugShowLineIntersection(pts, wt, wn, ts);
                            break;
                        case SkIntersectionHelper::kLine_Segment:
                ...
                case SkIntersectionHelper::kConic_Segment:
                    switch (wn.segmentType()) {
                        ...
                        case SkIntersectionHelper::kCubic_Segment: {
                            swap = true;
                            pts = ts.intersect(
                                    cubic2.set(wn.pts() PkDEBUGPARAMS(ts.globalState())),
                                    conic1.set(wt.pts(),
                                               wt.weight() PkDEBUGPARAMS(ts.globalState())));
                ...
                case SkIntersectionHelper::kCubic_Segment:
                    switch (wn.segmentType()) {
                        ...
                        case SkIntersectionHelper::kConic_Segment: {
                            pts = ts.intersect(
                                    cubic1.set(wt.pts() PkDEBUGPARAMS(ts.globalState())),
                                    conic2.set(wn.pts(),
                                               wn.weight() PkDEBUGPARAMS(ts.globalState())));
