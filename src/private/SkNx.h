...
template <int N, typename T> struct SkNx {
    ...
    AI T operator[](int k) const { return k < N / 2 ? fLo[k] : fHi[k - N / 2]; }
...
template <typename T> struct SkNx<1, T> {
    ...
    AI T operator[](int PkDEBUGCODE(k)) const { return fVal; }
...
#include "src/private/SkNx_sse.h"
#elif !defined(SKNX_NO_SIMD) && defined(PK_ARM_HAS_NEON)
#include "src/private/SkNx_neon.h"
#else
