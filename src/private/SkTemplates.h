...
public:
     ...
     */
    T& operator[](int index) const { return fArray[index]; }
...
private:
    ...
    // Stack frame size is limited for PK_BUILD_FOR_GOOGLE3. 4k is less than the actual max, but
...
constexpr auto SkMakeArrayFromIndexSequence(C c, std::index_sequence<Is...> is)
        -> std::array<decltype(c(std::declval<typename decltype(is)::value_type>())),
                      sizeof...(Is)> {
    return {{c(Is)...}};
}
